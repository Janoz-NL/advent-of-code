...
public class Day13 {
    ...
    public static final Packet DIV_1 = new Packet("[[2]]");
    public static final Packet DIV_2= new Packet("[[6]]");
    ...
    static void both(String file) {
        ...
        while (iterator.hasNext()) {
            i++;
            p1 = new Packet(iterator.next());
            p2 = new Packet(iterator.next());
    ...
    static void part2(String file) {
        ...
        SortedSet<Packet> packages = new TreeSet<>();

        packages.add(DIV_1);
        packages.add(DIV_2);

        while (iterator.hasNext()) {
            packages.add(new Packet(iterator.next()));
            packages.add(new Packet(iterator.next()));
            if (iterator.hasNext()) iterator.next();
        }
    ...
    static void compare(String line) {
        String processed = new Packet(line).asString();
